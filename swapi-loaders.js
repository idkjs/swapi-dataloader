...
export type LoadersType = $ReadOnly<{|
  ...
  getVehicles: DataLoader<
    ...
    string
  >,
  getStarships: DataLoader<
    {|
      ...$Diff<
        $Call<ExtractArg, [$PropertyType<ResourcesType, "getStarships">]>,
        {
          starship_ids: $PropertyType<
            $Call<ExtractArg, [$PropertyType<ResourcesType, "getStarships">]>,
            "starship_ids"
          >
        }
    ...
    $ElementType<
      $Call<
        ExtractPromisedReturnValue<
          [$Call<ExtractArg, [$PropertyType<ResourcesType, "getStarships">]>]
        >,
        $PropertyType<ResourcesType, "getStarships">
      >,
      0
    >,
    // This third argument is the cache key type. Since we use objectHash in cacheKeyOptions, this is "string".
    string
  >,
  getSpecies: DataLoader<
    {|
      ...$Diff<
        $Call<ExtractArg, [$PropertyType<ResourcesType, "getSpecies">]>,
        {
          species_ids: $PropertyType<
            $Call<ExtractArg, [$PropertyType<ResourcesType, "getSpecies">]>,
            "species_ids"
          >
        }
    ...
    $ElementType<
      $Call<
        ExtractPromisedReturnValue<
          [$Call<ExtractArg, [$PropertyType<ResourcesType, "getSpecies">]>]
        >,
        $PropertyType<ResourcesType, "getSpecies">
      >,
      0
    >,
    // This third argument is the cache key type. Since we use objectHash in cacheKeyOptions, this is "string".
...
export default function getLoaders(
  ...
  return Object.freeze({
    ...
    >(
      /**
       * ===============================================================
       * Generated DataLoader: getStarships
       * ===============================================================
       *
       * Resource Config:
       *
       * ```json
       * {
       *   "docsLink": "https://swapi.dev/documentation#starships",
       *   "isBatchResource": true,
       *   "batchKey": "starship_ids",
       *   "newKey": "starship_id"
       * }
       * ```
       */
      async keys => {
        invariant(
          typeof resources.getStarships === "function",
          [
            "[dataloader-codegen :: getStarships] resources.getStarships is not a function.",
            'Did you pass in an instance of getStarships to "getLoaders"?'
          ].join(" ")
        );
         ...
         * We'll refer to each element in the group as a "request ID".
         */
        const requestGroups = partitionItems("starship_id", keys);
        ...
        const groupedResults = await Promise.all(
          requestGroups.map(async requestIDs => {
            ...
            const resourceArgs = [
              {
                ..._.omit(requests[0], "starship_id"),
                ["starship_ids"]: requests.map(k => k["starship_id"])
              }
            ];
            ...
            let response = await (async _resourceArgs => {
              ...
              try {
                // Finally, call the resource!
                _response = await resources.getStarships(...__resourceArgs);
              } catch (error) {
                const errorHandler =
                  options && typeof options.errorHandler === "function"
                    ? options.errorHandler
                 ...
                 * keys requested. We can do slightly better by returning the error object for just the keys in this batch request.
                 */
                _response = await errorHandler(["getStarships"], error);

                // Check that errorHandler actually returned an Error object, and turn it into one if not.
                if (!(_response instanceof Error)) {
                  _response = new Error(
                    [
                      `[dataloader-codegen :: getStarships] Caught an error, but errorHandler did not return an Error object.`,
                      `Instead, got ${typeof _response}: ${util.inspect(
                        _response
                      )}`
                    ].join(" ")
            ...
            if (!(response instanceof Error)) {
              if (!Array.isArray(response)) {
                response = new Error(
                  [
                    "[dataloader-codegen :: getStarships]",
                    "Expected response to be an array!"
                  ].join(" ")
                );
              }
            ...
            if (!(response instanceof Error)) {
              ...
              if (response.length !== requests.length) {
                ...
                response = new BatchItemNotFoundError(
                  [
                    `[dataloader-codegen :: getStarships] Resource returned ${response.length} items, but we requested ${requests.length} items.`,
                    "Add reorderResultsByKey to the config for this resource to be able to handle a partial response."
                  ].join(" ")
                );
            ...
            if (response instanceof Error) {
              response = requestIDs.map(requestId => {
                ...
                return new CaughtResourceError(
                  `[dataloader-codegen :: getStarships] Caught error during call to resource. Error: ${response.stack}`,
                  response,
                  reorderResultsByValue
                );
              });
    ...
    >(
      /**
       * ===============================================================
       * Generated DataLoader: getSpecies
       * ===============================================================
       *
       * Resource Config:
       *
       * ```json
       * {
       *   "docsLink": "https://swapi.dev/documentation#species",
       *   "isBatchResource": true,
       *   "batchKey": "species_ids",
       *   "newKey": "species_id"
       * }
       * ```
       */
      async keys => {
        invariant(
          typeof resources.getSpecies === "function",
          [
            "[dataloader-codegen :: getSpecies] resources.getSpecies is not a function.",
            'Did you pass in an instance of getSpecies to "getLoaders"?'
          ].join(" ")
        );
         ...
         * We'll refer to each element in the group as a "request ID".
         */
        const requestGroups = partitionItems("species_id", keys);
        ...
        const groupedResults = await Promise.all(
          requestGroups.map(async requestIDs => {
            ...
            const resourceArgs = [
              {
                ..._.omit(requests[0], "species_id"),
                ["species_ids"]: requests.map(k => k["species_id"])
              }
            ];
            ...
            let response = await (async _resourceArgs => {
              ...
              try {
                // Finally, call the resource!
                _response = await resources.getSpecies(...__resourceArgs);
              } catch (error) {
                const errorHandler =
                  options && typeof options.errorHandler === "function"
                    ? options.errorHandler
                 ...
                 * keys requested. We can do slightly better by returning the error object for just the keys in this batch request.
                 */
                _response = await errorHandler(["getSpecies"], error);

                // Check that errorHandler actually returned an Error object, and turn it into one if not.
                if (!(_response instanceof Error)) {
                  _response = new Error(
                    [
                      `[dataloader-codegen :: getSpecies] Caught an error, but errorHandler did not return an Error object.`,
                      `Instead, got ${typeof _response}: ${util.inspect(
                        _response
                      )}`
                    ].join(" ")
            ...
            if (!(response instanceof Error)) {
              ...
              if (response.length !== requests.length) {
                ...
                response = new BatchItemNotFoundError(
                  [
                    `[dataloader-codegen :: getSpecies] Resource returned ${response.length} items, but we requested ${requests.length} items.`,
                    "Add reorderResultsByKey to the config for this resource to be able to handle a partial response."
                  ].join(" ")
                );
            ...
            if (response instanceof Error) {
              response = requestIDs.map(requestId => {
                ...
                return new CaughtResourceError(
                  `[dataloader-codegen :: getSpecies] Caught error during call to resource. Error: ${response.stack}`,
                  response,
                  reorderResultsByValue
                );
              });
